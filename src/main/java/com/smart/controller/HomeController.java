...
public class HomeController {
	...
	public String registerUser(@Valid @ModelAttribute User user, BindingResult result1 , @RequestParam(value = "agreement", defaultValue = "false") boolean agreement, Model model, HttpSession session) {
		...
		try {
			...
			if(!agreement) {
				System.out.println("You must Agree the terms and condition");
				throw new Exception("You must Agree the terms and condition");
			}
			
			if(result1.hasErrors())
			{
				System.out.println("Errors validation"+result1.toString());
